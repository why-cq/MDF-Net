...
from dataloder.Clip_coco_dataset import ClipCocoDataset
from dataloder.Clip_coco_dataset import ClipCocoDataset
import utils.samplers.distributed
import numpy as np
...
def sample_collate(batch):
    ...
    att_feats = torch.cat([torch.unsqueeze(torch.from_numpy(b),0) for b in att_feats], 0)
    att_mask = torch.ones(att_feats.shape[0],att_feats.shape[1])  # 这个7*7看grid的大小需要进行调整
...
def load_val(image_ids_path, gv_feat_path, att_feats_folder,extract_feature_files):
    ## 验证的时候这里需要修改，改为当前的dataset
    coco_set = ClipCocoDataset(
        ...
        extract_feature_files = extract_feature_files    #
        # extract_feature_files = cfg   clip提取的val特征
...
def load_val_cam(image_ids_path, gv_feat_path, att_feats_folder,extract_feature_files):
    ## 验证的时候这里需要修改，改为当前的dataset
    coco_set = ClipCocoDataset(
        image_ids_path=image_ids_path,
        input_seq=None,
