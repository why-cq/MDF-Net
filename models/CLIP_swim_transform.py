...
from models.encoder_decoder.CLIP_swim_encoder_decoder.CLIP_swim_encoder import WSA_Bolock, WSA_Bolock_v2
...
# 只使用了sw_block,并没有使用图像的全局特征
class CLIP_SWM_196(BasicModel):
    def __init__(self):
        super(CLIP_SWM_196, self).__init__()
    ...
    def forward(self, **kwargs):
        ...
        gv = kwargs[cfg.PARAM.GLOBAL_FEAT]
        ...
        gv = utils.expand_tensor(gv, cfg.DATA_LOADER.SEQ_PER_IMG)
        ...
        decoder_out = self.decoder(gv, seq, encoder_out, seq_mask, att_mask)
...
# 第二个版本加入对于全局特征的使用,这样做的
class CLIP_SWM_196_v2(BasicModel):
    def __init__(self):
        super(CLIP_SWM_196_v2, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
    ...
    def forward(self, **kwargs):
        ...
        # 编码器的输入为 batch_size*5 , 144 ,512
        encoder_out,gv = self.encoder(att_feats, gv)
        decoder_out = self.decoder(gv, seq, encoder_out, seq_mask, att_mask)
        return F.log_softmax(decoder_out, dim=-1)
    ...
    def decode_beam(self, **kwargs):
        ...
        # att_feats = self.backbone(att_feats)
        att_feats = self.att_embed(att_feats)
        encoder_out, gx = self.encoder(att_feats,gx)
        # p_att_feats = self.decoder.precompute(encoder_out)
