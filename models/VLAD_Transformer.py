...
class VLAD_Transformer_49(BasicModel):
    def __init__(self):
        super(VLAD_Transformer_49, self).__init__()
        ...
        self.encoder = VLAD_encoder(N=cfg.MODEL.BILINEAR.ENCODE_LAYERS)
    ...
    def forward(self, **kwargs):
        ...
        encoder_out, gx = self.encoder(gv_feature,att_feats)
...
class VLAD_Transformer_196(BasicModel):
    def __init__(self):
        super(VLAD_Transformer_196, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
        self.backbone = NetVLAD()
        ...
        # 在使用CLIP-L_14的全局特征的时候变为512
        self.global_change = nn.Linear(768, 512)
    ...
    def forward(self, **kwargs):
        ...
        # gv_feature为  B * 512
        gv_feature = kwargs[cfg.PARAM.GLOBAL_FEAT]
        if gv_feature.shape[1] == 768:
            gv_feature = self.global_change(gv_feature)
...
# todo:
class VLAD_Transformer_L_256(BasicModel):
    def __init__(self):
        super(VLAD_Transformer_L_256, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
        self.backbone = NetVLAD(input_dim=1024)
    ...
    def forward(self, **kwargs):
        ...
        # gv_feature为  B * 512
        gv_feature = kwargs[cfg.PARAM.GLOBAL_FEAT]
        gv_feature = self.global_embed(gv_feature)
