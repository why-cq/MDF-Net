...
class Cross_Decoder_v2(nn.Module):
    def __init__(
            self,
            vocab_size,
            embed_dim=512,
        ...
        super(Cross_Decoder_v2, self).__init__()
    ...
    def forward(self, gx, seq, encoder_out, seq_mask=None, att_mask=None):
        ...
        # cls_text = x[:, 1, :]
        # todo: 2.在进行图像和文本的交叉注意力 ,不要全局特征
        x = x[:,1:,:]

        for layer in self.cross_layers:
            x = layer(x, encoder_out, gx, att_mask)
...
class CrossDecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_out, gx, att_mask=None):
        # 在进入cross前,先将图像全局特征融合进来
