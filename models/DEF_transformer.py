...
from models.encoder_decoder.DFT_encoder_decoder.DFT_CLIP_encoder import DEF_Encoder
# from models.encoder_decoder.PureT_encoder import Encoder
from models.encoder_decoder.VLAD_encoder_decoder.VLAD_encoder import VLAD_encoder, NetVLAD
...
class DEF_Transformer_49(BasicModel):
    def __init__(self):
        super(DEF_Transformer_49, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
        self.backbone = NetVLAD()
        ...
        self.encoder = DEF_Encoder(N = cfg.MODEL.BILINEAR.ENCODE_LAYERS)
    ...
    def forward(self, **kwargs):
        ...
        # # 首先进入VLADnet 出来的grid_feature: B*(49+9)*512
        att_feats,_ = self.backbone(att_feats)
        # gv_feature为  B * 512
        gv_feature = kwargs[cfg.PARAM.GLOBAL_FEAT]

        # att_mask for features
        att_mask = kwargs[cfg.PARAM.ATT_FEATS_MASK]
        att_mask = utils.expand_tensor(att_mask, cfg.DATA_LOADER.SEQ_PER_IMG)
        att_feats = utils.expand_tensor(att_feats, cfg.DATA_LOADER.SEQ_PER_IMG)
        gv_feature = utils.expand_tensor(gv_feature, cfg.DATA_LOADER.SEQ_PER_IMG)
        ...
        # att_feats = self.att_embed(att_feats)   在vladnet中已经做了

        # 编码器的输入为 batch_size*5 , 49+9 ,512
        grid_feature = att_feats[:,:49,:]
        region_feature  = att_feats[:,49:,:]

        grid_feature, region_feature , _ = self.encoder(grid_feature,region_feature)

        # 这里出来的时候region_feature不在进行使用,解码器只需要grid_feature


        decoder_out = self.decoder(gv_feature, seq, grid_feature, seq_mask, att_mask)
    ...
    def decode_beam(self, **kwargs):
        ...
        kwargs[cfg.PARAM.ATT_FEATS] = grid_feature
        kwargs[cfg.PARAM.GLOBAL_FEAT] = gv_feature
        ...
        for t in range(cfg.MODEL.SEQ_LEN):
            ...
            if t == 0:
                # expand input
                encoder_out = utils.expand_tensor(grid_feature, beam_size)
                gx = utils.expand_tensor(gv_feature, beam_size)
...
class DEF_Transformer_196(BasicModel):
    def __init__(self):
        super(DEF_Transformer_196, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
        self.backbone = NetVLAD()
    ...
    def forward(self, **kwargs):
        ...
        # 编码器的输入为 batch_size*5 , 196+9 ,512
        grid_feature = att_feats[:,:196,:]
        region_feature  = att_feats[:,196:,:]
