...
from evaluation.evaler import Evaler, CAM,CAM_emb
...
class Tester(object):
    ...
    def gn_cam(self, epoch):
        res = self.cam(self.model, 'test_' + str(epoch))
        print('测试图像的注意力可视化特征图生成')
        self.logger.info('######## Epoch ' + str(epoch) + ' ########')
        self.logger.info(str(res))
